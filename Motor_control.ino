...
const double TICKS_PER_METER = TICKS_PER_REVOLUTION / (2.0 * 3.141592654 * WHEEL_RADIUS); // Originally 2880
...
const int DRIFT_MULTIPLIERL = 178;
const int DRIFT_MULTIPLIERR = 178;
 
// Turning PWM output (0 = min, 255 = max for PWM values)
const int PWM_TURN = 120;
 
// Set maximum and minimum limits for the PWM values
const int PWM_MIN = 90; // about 0.1 m/s
...
void calc_pwm_values(const geometry_msgs::Twist& cmdVel) {
  ...
  pwmLeftReq = (K_P) * cmdVel.linear.x + b + c;
 ...
 if (cmdVel.angular.z !=0) {
    ...
    if (cmdVel.angular.z > 0.4) {
      pwmLeftReq = 0;
      pwmRightReq = PWM_TURN+30;
    }
    // Turn right    
    else if (cmdVel.angular.z < -0.4) {
     pwmLeftReq = PWM_TURN+30;
     pwmRightReq = 0;
    }
 }
  // Go straight
    ...
    double currDifference = velRightWheel - velLeftWheel;
    ...
    pwmLeftReq += (int)(avgDifference * DRIFT_MULTIPLIERL);
    pwmRightReq -= (int)(avgDifference * DRIFT_MULTIPLIERR);
  }
